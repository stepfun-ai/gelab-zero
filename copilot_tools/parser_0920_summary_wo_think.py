...
class Parser0920Summary():
    ...
    def str2action(self, command_str):
        ...
        try:
            cot_part = command_str.split("</think>")[0].strip()
            kv_part = command_str.split("</think>")[1].strip()
        except IndexError:
            print(f"[Parser Warning] Missing <think> tags, treating entire response as kv")

            assert "\texplain:" in command_str, f"command_str {command_str} should contain explain field"
            cot_part = command_str.split("\texplain")[0].strip()
            kv_part = "explain:" + command_str.split("\texplain")[1].strip()
